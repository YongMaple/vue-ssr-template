...
// 配置跨域代理
// 微信2套接口地址：http://wx.17u.cn/wxflighttwotest
// 手Q接口地址：https://wx.17u.cn/qqflight

let apiUrl = 'http://tcflightfrontapi.t.17usoft.com/frontservice/'

if (isProd) {
  apiUrl = 'http://tcflightfrontapi.17usoft.com/frontservice/'
}

app.use(`${baseUrl}json`, proxy({
  target: apiUrl,
  changeOrigin: true,
  pathRewrite: {
    '^/qqflightnew/json' : '/json'
  }
}))
...
app.get(`${baseUrl}*`, (req, res) => {
  ...
  var s = Date.now()
  const context = { url: req.url.replace(new RegExp(baseUrl), ''), cookies: req.cookies }
